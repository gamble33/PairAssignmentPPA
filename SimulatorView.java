...
public class SimulatorView extends JFrame {
    ...
    public SimulatorView(int height, int width) {
        ...
        setColor(Seaweed.class, new Color(0, 130, 0));
        setColor(Whale.class, Color.blue);
        setColor(PinkDolphin.class, Color.pink);
        setColor(Turtle.class, new Color(100, 30, 30));
     ...
     */
    public void showStatus(int step, Field field, WorldState worldState) {
        ...
        float factor;
        if (worldState.getTimeOfDay() == TimeOfDay.Day) factor = (float) worldState.getTime() / WorldState.DAY_LENGTH;
        else factor = 1f - (float) worldState.getTime() / WorldState.NIGHT_LENGTH;
        factor *= 0.5f;

        Color backgroundColor = dimColor(EMPTY_COLOR, factor);
        stepLabel.setBackground(backgroundColor);
        population.setBackground(backgroundColor);
        ...
        for (int row = 0; row < field.getDepth(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                ...
                if (animal != null) {
                    ...
                    cellColor = getColor(animal.getClass());
                } else {
                    cellColor = EMPTY_COLOR;
                }
                cellColor = dimColor(cellColor, factor);
                fieldView.drawMark(col, row, cellColor);
            }
        }
        stats.countFinished();
